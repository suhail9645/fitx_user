...
class ExerciseOperationsImp {
  Future<Either<ErrorModel, Response>> getExerciseWithId(int id) async {
    try {
      SharedPreferences shrd = await SharedPreferences.getInstance();
      String access = shrd.getString('accessKey')!;
      Response response = await http.get(
          Uri.parse('${baseUrl}exercise/get/$id/'),
      ...
      if (response.statusCode == 200) {
        ...
        if (newAccess.isRight) {
          response = await http.get(Uri.parse('${baseUrl}exercise/get/$id/'),
